    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent
                ...
                className={cn(
                    "bg-white dark:bg-card transition-all duration-300 flex flex-col max-h-[90vh] sm:max-h-[85vh]",
                    ...
                    {step === 'result' && proposal && selectedGoal && (
                        <>
                            ...
                            <div className="md:hidden flex flex-col min-h-0 flex-1">
                                <Tabs defaultValue="strategy" className="w-full flex flex-col min-h-0">
                                    <TabsList className="grid w-full grid-cols-2 mb-4 shrink-0">
                                        <TabsTrigger value="strategy">Стратегія</TabsTrigger>
                                        <TabsTrigger value="context">Контекст</TabsTrigger>
                                    </TabsList>
                                    <TabsContent value="strategy" className="mt-0 space-y-6 overflow-y-auto max-h-[65vh] pb-6 pr-2">
                                        {strategyContent}
                                    </TabsContent>
                                    <TabsContent value="context" className="mt-0 space-y-6 rounded-2xl bg-slate-50 dark:bg-slate-900/50 p-4 border border-slate-100 dark:border-border overflow-y-auto max-h-[65vh]">
                                        {contextContent}
                                    </TabsContent>
                                </Tabs>
                            </div>
